...
function getClawdCommands(settings, event) {
  return getCommandHookEntries(settings, event, "nibbo-hook.js").map((hook) => hook.command);
}
...
describe("Hook installer version compatibility", () => {
  it("uses PowerShell-safe command hooks on Windows", () => {
    ...
    const stopHooks = getCommandHookEntries(settings, "Stop", "nibbo-hook.js");
  ...
  it("keeps remote hooks on the legacy bash-compatible format", () => {
    const hook = __test.buildCommandHookSpec("node", "/tmp/nibbo-hook.js", "Stop", {
      platform: "win32",
      remote: true,
    });
    ...
    assert.deepStrictEqual(hook, {
      type: "command",
      command: 'CLAWD_REMOTE=1 "node" "/tmp/nibbo-hook.js" Stop',
    });
  });
    ...
    const settingsPath = makeTempSettings({
      hooks: {
        StopFailure: [
          {
            matcher: "",
            hooks: [{ type: "command", command: 'node "/tmp/nibbo-hook.js" StopFailure' }],
          },
        ],
        PostCompact: [],
        PreCompact: [
  ...
  it("updates stale hook paths when command marker already exists", () => {
    const settingsPath = makeTempSettings({
      hooks: {
        Stop: [
          {
            matcher: "",
            hooks: [{ type: "command", command: 'node "/old/path/nibbo-hook.js" Stop' }],
          },
        ],
      },
    });
    ...
    assert.ok(commands[0].includes('hooks/nibbo-hook.js'));
        ...
        Stop: [
          {
            ...
            hooks: [{
              type: "command",
              shell: "powershell",
              command: '& "node" "/old/path/nibbo-hook.js" Stop',
            }],
          },
        ],
      },
    ...
    const settingsPath = makeTempSettings({
      hooks: {
        Stop: [
          {
            matcher: "",
            hooks: [{ type: "command", command: `"${existingAbsPath}" "/app/hooks/nibbo-hook.js" Stop` }],
          },
        ],
      },
    });
        ...
        Stop: [
          {
            ...
            hooks: [{
              type: "command",
              shell: "powershell",
              command: `& "${existingWinPath}" "C:/app/hooks/nibbo-hook.js" Stop`,
            }],
          },
        ],
      },
    ...
    const settingsPath = makeTempSettings({
      hooks: {
        Stop: [
          {
            matcher: "",
            hooks: [{ command: `"${existingAbsPath}" "/app/hooks/nibbo-hook.js" Stop` }],
          },
        ],
      },
    });
    ...
    const settingsPath = makeTempSettings({
      hooks: {
        WorktreeCreate: [
          {
            matcher: "",
            hooks: [{ type: "command", command: 'node "/tmp/nibbo-hook.js" WorktreeCreate' }],
          },
          {
            matcher: "",
    ...
    const settingsPath = makeTempSettings({
      hooks: {
        SessionStart: [
          ...
          {
            matcher: "",
            hooks: [{ type: "command", shell: "powershell", command: '& "node" "/tmp/nibbo-hook.js" SessionStart' }],
          },
          {
            matcher: "",
        ...
        Stop: [
          {
            matcher: "",
            hooks: [{ type: "command", shell: "powershell", command: '& "node" "/tmp/nibbo-hook.js" Stop' }],
          },
        ],
        PermissionRequest: [
          {
...
describe("async hook installer parity", () => {
  ...
  it("unregisterHooksAsync removes the same entries as unregisterHooks", async () => {
    const initial = {
      hooks: {
        Stop: [{ matcher: "", hooks: [{ type: "command", command: '"/usr/bin/node" "/tmp/nibbo-hook.js"' }] }],
