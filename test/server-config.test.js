...
describe("server-config helpers", () => {
  it("clearRuntimeConfig removes runtime.json when present", () => {
    ...
    fs.writeFileSync(runtimePath, JSON.stringify({ app: "nibbo", port: 23333 }));
  ...
  describe("resolveNodeBin on Windows", () => {
    ...
    it("rejects Nibbo.exe even when accessSync says it exists", () => {
      ...
      const clawdExe = "C:\\Program Files\\Nibbo\\Nibbo.exe";
      const result = serverConfig.resolveNodeBin({
        platform: "win32",
        env: WIN_ENV,
        execPath: clawdExe,
    ...
    it("validateWindowsNodeCandidate rejects Clawd, Electron, scoop shims, and non-node basenames", () => {
      ...
      assert.strictEqual(v("C:\\Program Files\\Nibbo\\Nibbo.exe"), null);
