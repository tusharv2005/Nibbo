...
describe("theme metadata facade helpers", () => {
  ...
  it("scans built-in and user metadata while skipping scaffold, malformed, and duplicate user themes", () => {
    ...
    writeTheme(builtinThemesDir, "nibbo", validThemeJson({ name: "Builtin Clawd" }));
    writeTheme(builtinThemesDir, "template", validThemeJson({ name: "Template", _scaffoldOnly: true }));
    writeTheme(userThemesDir, "nibbo", validThemeJson({ name: "User Clawd" }));
    ...
    assert.deepStrictEqual(themes.map((theme) => theme.id), ["nibbo", "user-cat"]);
    assert.strictEqual(themes.find((theme) => theme.id === "nibbo").name, "Builtin Clawd");
  });
});
