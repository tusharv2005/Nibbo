...
describe("doctor log opener", () => {
  ...
  it("resolves the newest allowed log across ~/.nibbo and userData", () => {
    ...
    const target = resolveNibboLogTarget({ homeDir, userDataDir });
  ...
  it("includes focus-debug.log in default log discovery", () => {
    ...
    const target = resolveNibboLogTarget({ homeDir: tmp, userDataDir });
  ...
  it("rejects path traversal requests", () => {
    const target = resolveNibboLogTarget({
  ...
  it("opens the fallback ~/.nibbo directory when no log exists", async () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "clawd-doctor-log-open-"));
    const opened = [];
    const result = await openNibboLog({
      homeDir: tmp,
      shell: {
    ...
    assert.strictEqual(opened[0], path.join(tmp, ".nibbo"));
