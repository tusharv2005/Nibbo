...
test("settings-tab-remote-ssh.js loads in a sandbox via the same IIFE pattern as siblings", () => {
  ...
  // IIFE registration check — must call NibboSettingsTabRemoteSsh = { init }.
  assert.match(code, /root\.NibboSettingsTabRemoteSsh\s*=\s*\{\s*init\s*\}/);
...
test("settings-tab-remote-ssh.js can be evaluated without DOM (no top-level DOM access)", () => {
  ...
  // to register `NibboSettingsTabRemoteSsh`; render() is what actually touches
  ...
  assert.ok(sandbox.globalThis.NibboSettingsTabRemoteSsh, "tab module must register on globalThis");
  assert.equal(typeof sandbox.globalThis.NibboSettingsTabRemoteSsh.init, "function");
});
