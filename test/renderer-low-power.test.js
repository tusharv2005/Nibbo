...
describe("renderer low-power idle mode", () => {
  ...
  it("relays low-power pauses to trusted scripted SVG runtimes", () => {
    ...
    assert.ok(source.includes("target.contentWindow.__nibboSetLowPowerPaused"));
...
describe("renderer glyph flip compensation", () => {
  ...
  it("notifies object-channel SVGs when mini-left glyph compensation changes", () => {
    ...
    assert.ok(source.includes("typeof svgWindow.__nibboSetGlyphFlipCompensation === \"function\""));
    assert.ok(source.includes("svgWindow.__nibboSetGlyphFlipCompensation(true);"));
    assert.ok(source.includes("svgWindow.__nibboSetGlyphFlipCompensation(false);"));
  });
});
