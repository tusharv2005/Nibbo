...
describe("package build config", () => {
  ...
  describe("Telegram approval sidecar packaging", () => {
    ...
    it("copies cc-connect-nibbo sidecars into packaged resources", () => {
      const extra = pkg.build.extraResources || [];
      const copied = extra.some(
        (e) => e && e.from === "bin/cc-connect-nibbo" && e.to === "sidecars/cc-connect-nibbo"
      );
      assert.ok(
        copied,
        "build.extraResources must copy bin/cc-connect-nibbo -> sidecars/cc-connect-nibbo"
      );
    });

    it("documents the expected sidecar binary names in the README", () => {
      const readme = path.join(ROOT, "bin", "cc-connect-nibbo", "README.md");
      assert.ok(fs.existsSync(readme), "bin/cc-connect-nibbo/README.md should document release binary names");
      const text = fs.readFileSync(readme, "utf8");
      assert.match(text, /windows-x64\/cc-connect-nibbo\.exe/);
      assert.match(text, /darwin-arm64\/cc-connect-nibbo/);
      assert.match(text, /linux-x64\/cc-connect-nibbo/);
    });
