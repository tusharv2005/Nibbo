...
test("main wires nibbo:// protocol dispatch through the Codex Pet importer", () => {
  ...
  assert.ok(runtimeSource.includes("codexPetImporter.parseNibboImportUrl(rawUrl)"));
...
test("package metadata registers the clawd protocol and exposes dev registration", () => {
  ...
  assert.ok(protocols.some((entry) => Array.isArray(entry.schemes) && entry.schemes.includes("nibbo")));
