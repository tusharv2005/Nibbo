...
function loadSettingsI18nBundleForTest() {
  ...
  return context.NibboSettingsI18n;
}
...
function loadSettingsCoreForTest(settingsAPI) {
  ...
  return context.NibboSettingsCore;
}
...
function loadGeneralLanguageRowForTest({
  ...
  const generalSource = fs.readFileSync(path.join(SRC_DIR, "settings-tab-general.js"), "utf8")
    .replace(
      "root.NibboSettingsTabGeneral = { init };",
      "root.NibboSettingsTabGeneral = { init, __test: { buildLanguageRow } };"
    );
  vm.runInContext(generalSource, context);

  const core = context.NibboSettingsCore;
  core.state.snapshot = snapshot || { lang: "en" };
  core.state.activeTab = "general";
  context.NibboSettingsTabGeneral.init(core);
  ...
  function renderLanguageOnly() {
    ...
    content.innerHTML = "";
    content.appendChild(context.NibboSettingsTabGeneral.__test.buildLanguageRow());
  }
...
function loadThemeTabForTest({
  ...
  context.NibboSettingsTabTheme.init(core);
...
function loadAgentsTabForTest({
  ...
  const localStorageData = {
    "nibbo.settings.collapsedGroups.v1": JSON.stringify(collapsedGroups),
  };
  ...
  context.NibboSettingsTabAgents.init(core);
...
function loadAnimMapTabForTest({
  ...
  const core = context.NibboSettingsCore;
  core.state.snapshot = snapshot || { theme: "nibbo", themeOverrides: {} };
  core.state.activeTab = "animMap";
  context.NibboSettingsTabAnimMap.init(core);
...
function loadTelegramApprovalTabForTest({
  ...
  context.NibboSettingsTabTelegramApproval.init(core);
  function render() {
    content.innerHTML = "";
...
function loadAnimOverridesTabForTest({
  ...
  const context = {
    ...
    globalThis: null,
    NibboSettingsAnimOverridesMerge: require(SETTINGS_ANIM_OVERRIDES_MERGE),
  };
  context.globalThis = context;
  vm.createContext(context);
  ...
  context.NibboSettingsTabAnimOverrides.init(core);
...
describe("settings renderer browser environment", () => {
  ...
  it("uses browser globals instead of CommonJS in settings renderer modules", () => {
    ...
    assert.ok(rendererSource.includes("globalThis.NibboSettingsCore"));
    assert.ok(coreSource.includes("NibboSettingsSizeSlider"));
    assert.ok(i18nSource.includes("globalThis"));
    assert.ok(doctorModalSource.includes("globalThis"));
    assert.ok(doctorModalSource.includes("NibboSettingsDoctorModal"));
  ...
  it("wires Nibbo Doctor through Settings with Step 2 connection actions", () => {
    const html = fs.readFileSync(SETTINGS_HTML, "utf8");
    const css = fs.readFileSync(SETTINGS_CSS, "utf8");
    const rendererSource = fs.readFileSync(SETTINGS_RENDERER, "utf8");
    const doctorModalSource = fs.readFileSync(SETTINGS_DOCTOR_MODAL, "utf8");
    ...
    assert.ok(rendererSource.includes("NibboSettingsDoctorModal.renderSidebarIndicator"));
    ...
    assert.ok(doctorModalSource.includes("root.doctor.openNibboLog"));
    ...
    assert.ok(doctorModalSource.includes('commandAction.type !== "restart-nibbo"'));
    ...
    assert.ok(preloadSource.includes('ipcRenderer.invoke("doctor:open-nibbo-log"'));
    ...
    assert.ok(doctorIpcSource.includes('ipcMain.handle("doctor:open-nibbo-log"'));
    ...
    assert.ok(doctorIpcSource.includes("openNibboLog"));
  ...
  it("groups Theme cards and exposes theme import actions in Settings", () => {
    ...
    assert.strictEqual(strings.en.themeImportUserThemeZip, "Import Nibbo theme package (.zip)");
    ...
    assert.strictEqual(strings.zh.themeImportUserThemeZip, "导入 Nibbo 主题包（.zip）");
  ...
  it("keeps Theme card footers reserved without leaking button keyboard events to card activation", async () => {
    const { content, commands } = loadThemeTabForTest({
      themes: [
        { id: "nibbo", name: "Nibbo", builtin: true, active: true },
  ...
  it("uses a dedicated Settings agent ordering helper before rendering Agent management groups", () => {
    ...
    assert.ok(agentsSource.includes("NibboSettingsAgentOrder"));
  ...
  it("keeps Animation Map theme override broadcasts in place and syncs the mounted switch", () => {
    ...
    harness.core.state.mountedControls.animMapSwitches.set("nibbo:error", {
      element: sw,
      themeId: "nibbo",
      stateKey: "error",
    });
