...
describe("installer default path exports", () => {
  it("exports default config locations for hook-based agents", () => {
    ...
    assert.strictEqual(pi.DEFAULT_EXTENSION_DIR, path.join(home, ".pi", "agent", "extensions", "nibbo"));

    assert.strictEqual(hermes.DEFAULT_PARENT_DIR, path.join(home, ".hermes"));
    assert.strictEqual(hermes.DEFAULT_PLUGIN_DIR, path.join(home, ".hermes", "plugins", "nibbo"));
  });
});
