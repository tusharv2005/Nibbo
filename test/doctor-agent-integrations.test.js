...
function geminiHooksConfig(commandForEvent = (event) => `"/node" "/app/hooks/gemini-hook.js" ${event}`) {
  ...
  for (const event of GEMINI_HOOK_EVENTS) {
    hooks[event] = [{
      matcher: "*",
      hooks: [{ name: "nibbo", type: "command", command: commandForEvent(event) }],
    }];
  }
  return hooks;
}
...
describe("checkAgentIntegrations", () => {
  ...
  it("warns when Gemini is missing any required hook event", () => {
    ...
    writeJson(descriptor.configPath, {
      hooks: {
        BeforeTool: [{
          matcher: "*",
          hooks: [{ name: "nibbo", type: "command", command: '"/node" "/app/hooks/gemini-hook.js" BeforeTool' }],
        }],
      },
    });
  ...
  it("turns Gemini ok into warning when hooksConfig.enabled=false", () => {
    ...
    assert.strictEqual(detail.status, "not-connected");
    assert.strictEqual(detail.level, "warning");
    assert.strictEqual(detail.detail, "Gemini hooks are disabled in settings.json; Nibbo preserves this user setting and will not receive hook events");
    assert.deepStrictEqual(detail.supplementary, {
      key: "gemini_hooks",
  ...
  it("warns when Qwen Code is missing any required hook event", () => {
    ...
    writeJson(descriptor.configPath, {
      hooks: {
        PreToolUse: [{
          matcher: "*",
          hooks: [{ name: "nibbo", type: "command", command: '"/node" "/app/hooks/qwen-code-hook.js" PreToolUse' }],
        }],
      },
    });
  ...
  it("does not offer automatic repair when Qwen hooks are disabled globally", () => {
    ...
    assert.strictEqual(detail.status, "not-connected");
    assert.strictEqual(detail.level, "warning");
    assert.strictEqual(detail.detail, "Qwen Code hooks are disabled in settings.json; Nibbo preserves this user setting and will not receive hook events");
    assert.deepStrictEqual(detail.supplementary, {
      key: "qwen_hooks",
      value: "disabled-global",
  ...
  it("does not offer automatic repair when Antigravity Clawd hooks are disabled", () => {
    ...
    assert.strictEqual(detail.status, "not-connected");
    assert.strictEqual(detail.detail, "Antigravity Nibbo hooks are disabled in hooks.json; Nibbo preserves this user setting and will not receive hook events");
    assert.deepStrictEqual(detail.supplementary, {
      key: "antigravity_hooks",
      value: "disabled-nibbo",
      detail: "nibbo.enabled is false",
    });
    assert.strictEqual(detail.fixAction, undefined);
  });
  ...
  function piDescriptor() {
    ...
    return baseDescriptor({
      ...
      parentDir,
      configPath: path.join(parentDir, "extensions", "nibbo"),
  ...
  function makeOpenClawPluginDir(root) {
    ...
    fs.writeFileSync(path.join(pluginDir, "index.js"), "export default { id: 'nibbo', register() {} };\n", "utf8");
  ...
  it("checks Hermes plugin directory files and enabled marker", () => {
    ...
    const pluginDir = path.join(parentDir, "plugins", "nibbo");
    const descriptor = baseDescriptor({
      agentId: "hermes",
      marker: "nibbo",
      parentDir,
  ...
  it("does not report Hermes ok when nibbo appears only in disabled plugins", () => {
    const root = makeTempDir();
    const parentDir = path.join(root, ".hermes");
    const pluginDir = path.join(parentDir, "plugins", "nibbo");
    const descriptor = baseDescriptor({
