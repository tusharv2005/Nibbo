...
describe("Kiro hook installer", () => {
  it("creates nibbo.json from kiro_default template without changing cli settings", () => {
    ...
    const clawdPath = path.join(agentsDir, "nibbo.json");
    assert.ok(fs.existsSync(clawdPath));

    const clawdAgent = readJson(clawdPath);
    assert.strictEqual(clawdAgent.name, "nibbo");
  ...
  it("reseeds legacy hook-only clawd agent from kiro_default template", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "nibbo",
        description: "Nibbo desktop pet hook integration",
        hooks: {
  ...
  it("re-syncs nibbo.json from the latest kiro_default template on every run", () => {
    const { agentsDir } = makeTempKiroHome();
    const clawdPath = path.join(agentsDir, "nibbo.json");
    fs.writeFileSync(
      clawdPath,
...
`, "utf8");
    ...
    // Name is always overridden to "nibbo"
    assert.strictEqual(agent.name, "nibbo");
    ...
    assert.strictEqual(agent.description, "Nibbo desktop pet hook integration");
  ...
  it("generated hook commands use PowerShell-safe format on Windows", () => {
    ...
    registerKiroHooks({
      ...
      syncClawdAgent(filePath) {
        fs.writeFileSync(
          filePath,
          JSON.stringify({ name: "nibbo", description: "x", hooks: {} }, null, 2),
          "utf8"
        );
      },
    });

    const clawdAgent = readJson(path.join(agentsDir, "nibbo.json"));
    for (const event of KIRO_HOOK_EVENTS) {
...
`, "utf8");
  ...
  it("fallback preserves existing nibbo.json when kiro-cli is unavailable", () => {
    const { agentsDir } = makeTempKiroHome();
    const clawdPath = path.join(agentsDir, "nibbo.json");

    // Pre-seed a full nibbo.json (simulates a prior successful sync)
    const preExisting = {
      name: "nibbo",
      description: "Nibbo desktop pet hook integration",
