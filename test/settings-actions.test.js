...
describe("updateRegistry pure-data validators", () => {
  ...
  it("theme validator requires a non-empty string", () => {
    ...
    const deps = { snapshot: baseSnapshot };
    assert.strictEqual(entry.validate("nibbo", deps).status, "ok");
  ...
  it("theme effect proxies to deps.activateTheme and maps throws to error", () => {
    ...
    const deps = {
      snapshot: { ...baseSnapshot, themeOverrides: { nibbo: overrideMap } },
      activateTheme: (id, variantId, targetOverrideMap) => {
        calls.push({ id, variantId, targetOverrideMap });
        if (id === "bad") throw new Error("boom");
      },
    };
    assert.deepStrictEqual(entry.effect("nibbo", deps), { status: "ok" });
  ...
  it("theme effect errors when activateTheme dep missing", () => {
    const entry = updateRegistry.theme;
    const result = entry.effect("nibbo", { snapshot: baseSnapshot });
  ...
  it("themeVariant requires a plain object (no effect runs)", () => {
    ...
    assert.strictEqual(updateRegistry.themeVariant({ nibbo: "chill" }, deps).status, "ok");
...
describe("doctor repair commands", () => {
  ...
  it("routes Doctor restart-nibbo repair through deps.restartNibbo", async () => {
    const calls = [];
    const r = await commandRegistry.repairDoctorIssue(
      { type: "restart-nibbo", confirmed: true },
      { restartNibbo: () => calls.push("restart") }
    );
  ...
  it("does not run Doctor restart-nibbo repair without confirmation", async () => {
    const calls = [];
    const r = await commandRegistry.repairDoctorIssue(
      { type: "restart-nibbo" },
      { restartNibbo: () => calls.push("restart") }
  ...
  it("returns an error when restart-nibbo is dispatched without deps.restartNibbo", async () => {
    const r = await commandRegistry.repairDoctorIssue({ type: "restart-nibbo", confirmed: true }, {});

    assert.strictEqual(r.status, "error");
    assert.match(r.message, /restartNibbo/);
  });
...
describe("removeTheme command", () => {
  ...
  function makeDeps(overrides = {}) {
    ...
    const deps = {
      ...
      getThemeInfo: (id) => {
        ...
        if (id === "nibbo") return { builtin: true, active: true };
  ...
  it("rejects built-in themes", async () => {
    ...
    const r = await commandRegistry.removeTheme("nibbo", deps);
    assert.strictEqual(r.status, "error");
  ...
  it("strips themeVariant entry on success when one exists", async () => {
    const snapshotWithVariant = {
      ...baseSnapshot,
      themeVariant: { cat: "chill", nibbo: "default" },
    };
    const { deps } = makeDeps({ snapshot: snapshotWithVariant });
    const r = await commandRegistry.removeTheme("cat", deps);
    assert.strictEqual(r.status, "ok");
    assert.ok(r.commit, "commit field expected");
    assert.deepStrictEqual(r.commit.themeVariant, { nibbo: "default" });
...
describe("setThemeSelection command", () => {
  ...
  it("rejects non-string variantId when provided", () => {
    const { deps } = makeDeps();
    const r = commandRegistry.setThemeSelection({ themeId: "nibbo", variantId: 42 }, deps);
    assert.strictEqual(r.status, "error");
  });

  it("accepts string payload as themeId shorthand", () => {
    const { deps, calls } = makeDeps();
    const r = commandRegistry.setThemeSelection("nibbo", deps);
    assert.strictEqual(r.status, "ok");
    assert.strictEqual(calls.activateTheme.length, 1);
    assert.strictEqual(calls.activateTheme[0].themeId, "nibbo");
  ...
  it("uses snapshot.themeVariant when variantId not provided", () => {
    const snapshotWithVariant = { ...baseSnapshot, themeVariant: { nibbo: "chill" } };
    const { deps, calls } = makeDeps({ snapshot: snapshotWithVariant });
    const r = commandRegistry.setThemeSelection({ themeId: "nibbo" }, deps);
    assert.strictEqual(r.status, "ok");
  ...
  it("passes the target theme override map into activateTheme", () => {
    ...
    const snapshotWithOverride = {
      ...baseSnapshot,
      themeOverrides: { nibbo: overrideMap },
    };
  ...
  it("explicit variantId overrides snapshot map", () => {
    const snapshotWithVariant = { ...baseSnapshot, themeVariant: { nibbo: "chill" } };
    const { deps, calls } = makeDeps({ snapshot: snapshotWithVariant });
    const r = commandRegistry.setThemeSelection({ themeId: "nibbo", variantId: "hyper" }, deps);
    assert.strictEqual(r.status, "ok");
  ...
  it("commits theme + themeVariant atomically", () => {
    const { deps } = makeDeps();
    const r = commandRegistry.setThemeSelection({ themeId: "nibbo", variantId: "chill" }, deps);
    assert.strictEqual(r.status, "ok");
    assert.ok(r.commit, "commit field expected");
    assert.strictEqual(r.commit.theme, "nibbo");
    assert.deepStrictEqual(r.commit.themeVariant, { nibbo: "chill" });
  });
  ...
  it("preserves other themes' variantIds when committing", () => {
    ...
    const r = commandRegistry.setThemeSelection({ themeId: "nibbo", variantId: "chill" }, deps);
    assert.strictEqual(r.status, "ok");
    assert.deepStrictEqual(r.commit.themeVariant, { calico: "hyper", nibbo: "chill" });
  });
  ...
  it("self-heals by committing the RESOLVED variantId on dead-variant fallback", () => {
    ...
    const snapshotWithDead = { ...baseSnapshot, themeVariant: { nibbo: "dead" } };
  ...
  it("errors when activateTheme dep is missing", () => {
    const r = commandRegistry.setThemeSelection({ themeId: "nibbo" }, { snapshot: baseSnapshot });
    assert.strictEqual(r.status, "error");
...
describe("setAnimationOverride reaction slot", () => {
  const baseSnapshot = { theme: "nibbo", themeOverrides: {} };
  ...
  it("accepts valid reactionKey and writes reactions.<key>.file", () => {
    ...
    assert.deepStrictEqual(
      r.commit.themeOverrides.nibbo.reactions.clickLeft,
      { file: "my-poke.svg" }
    );
  });
  ...
  it("accepts durationMs for clickLeft reaction", () => {
    ...
    assert.strictEqual(r.status, "ok");
    assert.strictEqual(r.commit.themeOverrides.nibbo.reactions.clickLeft.durationMs, 3000);
  });
  ...
  it("clears reaction override when file is set to null with no other fields", () => {
    const snapshot = {
      theme: "nibbo",
      themeOverrides: { nibbo: { reactions: { clickLeft: { file: "old.svg" } } },
      },
    };
    const r = commandRegistry.setAnimationOverride({
      themeId: "nibbo",
    ...
    assert.strictEqual(r.commit.themeOverrides.nibbo, undefined);
  });

  it("preserves existing hitbox overrides when editing a reaction slot", () => {
    const snapshot = {
      theme: "nibbo",
      themeOverrides: { nibbo: {
    ...
    assert.strictEqual(r.status, "ok");
    assert.deepStrictEqual(r.commit.themeOverrides.nibbo.hitbox, {
...
describe("setSoundOverride command", () => {
  ...
  it("rejects missing themeId / soundName", () => {
    ...
    r = commandRegistry.setSoundOverride({ themeId: "nibbo", file: "a.mp3" }, noopDeps);
    assert.strictEqual(r.status, "error");
  ...
  it("rejects file when it is not null and not a non-empty string", () => {
    const r = commandRegistry.setSoundOverride(
      { themeId: "nibbo", soundName: "complete", file: "" },
      noopDeps
    );
    assert.strictEqual(r.status, "error");
  ...
  it("writes { sounds: { complete: { file } } } on first override", () => {
    const r = commandRegistry.setSoundOverride(
      { themeId: "nibbo", soundName: "complete", file: "my-complete.mp3" },
      noopDeps
    );
    assert.strictEqual(r.status, "ok");
    assert.deepStrictEqual(r.commit.themeOverrides.nibbo.sounds, {
  ...
  it("preserves originalName in the committed entry when provided", () => {
    const r = commandRegistry.setSoundOverride(
      { themeId: "nibbo", soundName: "complete", file: "complete.mp3", originalName: "cat-demo.mp3" },
      noopDeps
    );
    assert.strictEqual(r.status, "ok");
    assert.deepStrictEqual(r.commit.themeOverrides.nibbo.sounds, {
  ...
  it("null file clears the entry and removes the theme row when nothing else is overridden", () => {
    const snapshot = {
      theme: "nibbo",
      themeOverrides: { nibbo: { sounds: { complete: { file: "old.mp3" } } },
      },
    };
    const r = commandRegistry.setSoundOverride(
      { themeId: "nibbo", soundName: "complete", file: null },
      { snapshot, activateTheme: () => {} }
    );
    assert.strictEqual(r.status, "ok");
    assert.strictEqual(r.commit.themeOverrides.nibbo, undefined);
  ...
  it("preserves unrelated soundName entries when editing one", () => {
    ...
    const r = commandRegistry.setSoundOverride(
      { themeId: "nibbo", soundName: "complete", file: "new-c.mp3" },
      { snapshot, activateTheme: () => {} }
    );
    assert.strictEqual(r.status, "ok");
    assert.deepStrictEqual(r.commit.themeOverrides.nibbo.sounds, {
  ...
  it("preserves existing animation overrides when editing a sound slot", () => {
    ...
    const r = commandRegistry.setSoundOverride(
      { themeId: "nibbo", soundName: "complete", file: "done.mp3" },
      { snapshot, activateTheme: () => {} }
    );
    assert.strictEqual(r.status, "ok");
    const nextClawd = r.commit.themeOverrides.nibbo;
  ...
  it("same value is a noop (no commit)", () => {
    const snapshot = {
      theme: "nibbo",
      themeOverrides: { nibbo: { sounds: { complete: { file: "same.mp3" } } },
      },
    };
    const r = commandRegistry.setSoundOverride(
      { themeId: "nibbo", soundName: "complete", file: "same.mp3" },
      { snapshot, activateTheme: () => {} }
    );
    assert.strictEqual(r.status, "ok");
    assert.strictEqual(r.noop, true);
  ...
  it("when active theme changes, calls activateTheme with the new override map", () => {
    ...
    const calls = [];
    const r = commandRegistry.setSoundOverride(
      { themeId: "nibbo", soundName: "complete", file: "a.mp3" },
      {
        snapshot,
  ...
  it("active theme edit without activateTheme dep returns error", () => {
    const r = commandRegistry.setSoundOverride(
      { themeId: "nibbo", soundName: "complete", file: "a.mp3" },
      { snapshot: { theme: "nibbo", themeOverrides: {} } }
    );
    assert.strictEqual(r.status, "error");
    assert.match(r.message, /activateTheme/);
  });
...
describe("setWideHitboxOverride command", () => {
  it("rejects missing file / themeId", () => {
    const r = commandRegistry.setWideHitboxOverride({ themeId: "nibbo", enabled: true }, { snapshot: {} });
    assert.strictEqual(r.status, "error");
    assert.match(r.message, /file/);
  });
  ...
  it("rejects non-boolean / non-null enabled", () => {
    const r = commandRegistry.setWideHitboxOverride({
      themeId: "nibbo", file: "x.svg", enabled: "yes",
    }, { snapshot: {} });
    assert.strictEqual(r.status, "error");
  ...
  it("writes hitbox.wide[file] = true when enabled", () => {
    const snapshot = { theme: "nibbo", themeOverrides: {} };
    const r = commandRegistry.setWideHitboxOverride(
      { themeId: "nibbo", file: "clawd-error.svg", enabled: true },
      { snapshot, activateTheme: () => {} }
    );
    assert.strictEqual(r.status, "ok");
    assert.deepStrictEqual(
      r.commit.themeOverrides.nibbo.hitbox.wide,
  ...
  it("clears the entry when enabled=null (fall back to theme default)", () => {
    const snapshot = {
      theme: "nibbo",
      themeOverrides: { nibbo: { hitbox: { wide: { "clawd-error.svg": true } } },
      },
    };
    const r = commandRegistry.setWideHitboxOverride(
      { themeId: "nibbo", file: "clawd-error.svg", enabled: null },
      { snapshot, activateTheme: () => {} }
    );
    assert.strictEqual(r.status, "ok");
  ...
  it("triggers activateTheme with next override map when active theme changes", () => {
    ...
    const snapshot = { theme: "nibbo", themeOverrides: {} };
    const r = commandRegistry.setWideHitboxOverride(
      { themeId: "nibbo", file: "foo.svg", enabled: true },
      {
        snapshot,
    ...
    assert.strictEqual(r.status, "ok");
    assert.strictEqual(activatedWith.id, "nibbo");
...
describe("theme override subtree preservation", () => {
  it("setThemeOverrideDisabled keeps existing reactions and hitbox overrides", () => {
    ...
    const r = commandRegistry.setThemeOverrideDisabled(
      { themeId: "nibbo", stateKey: "attention", disabled: true },
      { snapshot }
    );
    assert.strictEqual(r.status, "ok");
    assert.deepStrictEqual(r.commit.themeOverrides.nibbo.reactions, {
...
describe("importAnimationOverrides command", () => {
  ...
  it("rejects when normalized payload has no valid entries", () => {
    const r = commandRegistry.importAnimationOverrides(
      { version: 1, themes: { nibbo: { not_a_real_field: 1 } } },
      { snapshot: {} }
    );
    assert.strictEqual(r.status, "error");
  ...
  it("merges by theme id into existing overrides by default", () => {
    ...
    assert.ok(r.commit.themeOverrides.nibbo, "nibbo overrides added on merge");
  ...
  it("replaces the entire map when mode=replace", () => {
    ...
    assert.ok(r.commit.themeOverrides.nibbo);
  });
  ...
  it("calls activateTheme with the new override map for the active theme", () => {
    ...
    assert.strictEqual(r.status, "ok");
    assert.strictEqual(calls.length, 1);
    assert.strictEqual(calls[0].id, "nibbo");
    assert.ok(calls[0].overrideMap, "overrideMap must not be null");
    assert.deepStrictEqual(
      calls[0].overrideMap,
      r.commit.themeOverrides.nibbo,
