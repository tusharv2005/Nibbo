...
describe("Gemini hook installer", () => {
  it("registers all events on fresh install", () => {
    ...
    for (const event of GEMINI_HOOK_EVENTS) {
      ...
      assert.strictEqual(hook.name, "nibbo");
  ...
  it("updates stale hook paths", () => {
    const settingsPath = makeTempSettingsFile({
      hooks: {
        AfterTool: [{ type: "command", command: '"/old/node" "/old/path/gemini-hook.js"', name: "nibbo" }],
      },
    });
  ...
  it("migrates stale flat Clawd entries into nested Gemini hook shape", () => {
    const settingsPath = makeTempSettingsFile({
      hooks: {
        BeforeTool: [
          { type: "command", command: '"/old/node" "/old/path/gemini-hook.js"', name: "nibbo" },
        ],
      },
    });
    ...
    assert.strictEqual(settings.hooks.BeforeTool[0].hooks[0].name, "nibbo");
  ...
  it("preserves existing node path when detection fails", () => {
    const settingsPath = makeTempSettingsFile({
      hooks: {
        BeforeTool: [{ type: "command", command: '"/home/user/.nvm/versions/node/v20/bin/node" "/some/path/gemini-hook.js"', name: "nibbo" }],
      },
    });
    ...
    const settingsPath = makeTempSettingsFile({
      hooks: {
        BeforeTool: [
          ...
          {
            matcher: "*",
            hooks: [{ type: "command", command: '"/stale/node" "/stale/path/gemini-hook.js"', name: "nibbo" }],
          },
        ],
      },
    });
  ...
  it("migrates legacy disabled Gemini hook command entries to clawd", () => {
    ...
    const settings = readJson(settingsPath);
    assert.deepStrictEqual(settings.hooksConfig.disabled, ["other-hook", "nibbo"]);
  });
