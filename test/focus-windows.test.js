...
describe("Windows terminal focus", () => {
  ...
  it("requires unique title matches for Windows Terminal parent and fallback windows", () => {
    ...
    try {
      ...
      assert.match(cmd, /Get-NibboVisiblePidWindows/);
      assert.match(cmd, /Get-NibboWindowsTerminalWindows/);
  ...
  it("reports Windows helper results through stdout instead of writing logs directly", () => {
    ...
    try {
      ...
      assert.match(cmd, /Write-NibboFocusResult/);
  ...
  it("caches successful Windows focus HWNDs by session key", () => {
    ...
    try {
      ...
      assert.match(cmd, /\$global:NibboFocusWindowCache/);
      assert.match(cmd, /Get-NibboCachedWindow/);
      assert.match(cmd, /reason = 'cached-window'/);
      assert.match(cmd, /Save-NibboFocusCache \$matches\[0\]/);
      assert.match(cmd, /Save-NibboFocusCache \$wtMatches\[0\]/);
      assert.match(cmd, /Save-NibboFocusCache \$singleWtWindows\[0\]/);
    } finally {
      cleanup();
    }
  });
  ...
  it("focuses a hook-captured Windows Terminal HWND before guessing by title", () => {
    ...
    try {
      ...
      assert.match(cmd, /Save-NibboFocusCache \$wtHwndFromHook/);
