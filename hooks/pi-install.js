...
const EXTENSION_DIR_NAME = "nibbo";
...
function registerPiExtension(options = {}) {
  ...
  if (!parentExists && !hasPiCommand(options)) {
    if (!options.silent) {
      console.log("Nibbo: Pi not found - skipping Pi extension registration");
    }
  ...
  if (extensionExists && !isManagedMarker(readJsonIfPresent(markerPath, fsImpl))) {
    if (!options.silent) {
      console.log(`Nibbo: ${extensionDir} exists but is not Clawd-managed - skipping`);
    }
...
function unregisterPiExtension(options = {}) {
  ...
  if (!dirExists(extensionDir, fsImpl)) {
    if (!options.silent) console.log("Nibbo: Pi extension is not installed");
    return { removed: false, skipped: true, reason: "missing", extensionDir };
  }
  if (!isManagedMarker(marker)) {
    if (!options.silent) console.log(`Nibbo: ${extensionDir} is not Clawd-managed - skipping uninstall`);
  ...
  if (!options.silent) console.log(`Nibbo: removed Pi extension from ${extensionDir}`);
