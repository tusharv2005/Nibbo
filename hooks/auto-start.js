#!/usr/bin/env node
// Nibbo Desktop Pet — Auto-Start Script
// Registered as a SessionStart hook BEFORE nibbo-hook.js.
  ...
  try {
    if (isPackaged) {
      if (isWin) {
        ...
        // exe:       <install>/Nibbo.exe
        const installDir = path.resolve(__dirname, "..", "..", "..");
        const exe = path.join(installDir, "Nibbo.exe");
        ...
        // deb/dir:  executable is <install>/nibbo, same depth as Windows.
        //   __dirname: <install>/resources/app.asar.unpacked/hooks
        //   install:   3 levels up
        ...
        if (appImage) {
          ...
          const installDir = path.resolve(__dirname, "..", "..", "..");
          const exe = path.join(installDir, "nibbo");
          spawn(exe, [], { detached: true, stdio: "ignore" }).unref();
        }
      }
    } else {
    ...
    process.stderr.write(`nibbo auto-start: ${err.message}\n`);
  }
}
