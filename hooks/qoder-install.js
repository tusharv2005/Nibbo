...
function buildQoderHookEntry(command) {
  return {
    matcher: "*",
    hooks: [{ name: "nibbo", type: "command", command }],
  };
}
...
// Qoder's `hooksConfig.disabled` list can name a hook group by id ("nibbo") or
// by raw command. Collapse Clawd command references into the "nibbo" id and
...
function registerQoderHooks(options = {}) {
  ...
  if (!options.settingsPath && !fs.existsSync(qoderDir)) {
    if (!options.silent) console.log("Nibbo: ~/.qoder/ not found — skipping Qoder hook registration");
