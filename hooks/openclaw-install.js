...
function registerOpenClawPlugin(options = {}) {
  ...
  if (!stateDirExists && !configFileExists && !getCommandAvailable()) {
    if (!options.silent) console.log("Nibbo: OpenClaw not found - skipping OpenClaw plugin registration");
    return {
      installed: false,
      skipped: true,
      updated: false,
  ...
  if (!configFileExists) {
    ...
    if (!options.silent) {
      console.log(`Nibbo: ${configPath} missing - skipping OpenClaw plugin registration`);
    }
    return {
      installed: false,
      skipped: true,
