...
**Pi** — uses a global extension directory at `~/.pi/agent/extensions/nibbo`. Clawd auto-registers it on launch when Pi is installed, or you can run `npm run install:pi-extension` manually. Interactive Pi sessions report lifecycle and tool activity to Clawd, but Pi is state-only: Clawd does not show permission bubbles, does not call Pi terminal confirmation, and preserves Pi's default YOLO execution behavior.
...
> Thanks to [@Magic-Bytes](https://github.com/Magic-Bytes) for the original SSH tunneling idea ([#9](https://github.com/rullerzhou-afk/nibbo/issues/9)).
...
> **Tip:** If the repo is cloned inside WSL (e.g. `~/nibbo`), hook scripts will automatically use WSL's Node.js path. If the repo is on a Windows drive (e.g. `/mnt/c/...`), make sure `node` is in WSL's `PATH`.

## Windows Notes

- **Installer**: GitHub Releases provide separate NSIS installers for Windows x64 and Windows ARM64. Use `Nibbo-Setup-<version>-x64.exe` on Intel/AMD Windows, and `Nibbo-Setup-<version>-arm64.exe` on Windows on ARM.
...
- **Packages**: AppImage and `.deb` are available from [GitHub Releases](https://github.com/rullerzhou-afk/nibbo/releases). After deb install, the app icon appears in GNOME's app menu.
