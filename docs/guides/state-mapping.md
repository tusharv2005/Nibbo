...
Pi uses a global extension (`~/.pi/agent/extensions/nibbo`) and maps interactive-session lifecycle events to shared Clawd states:
...
Pi is state-only in Nibbo: Clawd does not intercept permissions or add confirmation prompts, so Pi keeps its default YOLO execution behavior.
