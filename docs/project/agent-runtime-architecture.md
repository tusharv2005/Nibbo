...
Kiro CLI 状态同步（per-agent hook，stdin JSON）：
  ...
  custom agent 配置里，并额外维护一个 "nibbo" agent（继承 kiro_default，启动时从 kiro_default
...
Pi 状态同步（global extension，state-only）：
  Pi 触发 session_start / before_agent_start / tool_call / tool_result / agent_end 等事件
    → ~/.pi/agent/extensions/nibbo/index.ts（Pi extension runtime）
...
- Hermes config.yaml 是用户 YAML，不做 line-oriented 编辑；安装只复制托管 plugin 文件并调用 `hermes plugins enable nibbo`

## Pi Notes

- Pi 使用 global extension 目录 `~/.pi/agent/extensions/nibbo`；安装器复制 `pi-extension.ts` 和自包含的 `pi-extension-core.js`
