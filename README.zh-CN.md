...
- **Pi** — 通过全局 extension 集成，写入 `~/.pi/agent/extensions/nibbo`（Clawd 启动时自动注册，或执行 `npm run install:pi-extension`）；仅同步交互式 Pi 会话生命周期和工具活动状态，并保留 Pi 默认 YOLO 行为
...
普通用户建议直接从 **[GitHub Releases](https://github.com/rullerzhou-afk/nibbo/releases/latest)** 下载最新预构建安装包：

- **Windows**：`Nibbo-Setup-<version>-x64.exe` 或 `Nibbo-Setup-<version>-arm64.exe`
- **macOS**：`.dmg`
...
Nibbo 是一个社区驱动的项目。欢迎提 Bug、提需求、提 PR —— 在 [Issues](https://github.com/tusharv2005/Nibbo/issues) 里聊或直接提交 PR。
