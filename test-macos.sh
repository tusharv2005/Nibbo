#!/bin/bash
# macOS Adaptation Test Script for Nibbo
...
detect_port() {
  ...
  for port in "${candidates[@]}"; do
    if curl -s -D - -o /dev/null --connect-timeout 1 "http://127.0.0.1:$port/state" 2>/dev/null | tr -d '\r' | grep -iq '^x-clawd-server: nibbo$'; then
      echo "$port"
      return 0
    fi
  done
