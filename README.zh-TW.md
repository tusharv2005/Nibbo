...
- **Pi** — 以全域擴充功能整合，寫入 `~/.pi/agent/extensions/nibbo`（Clawd 啟動時自動註冊，或執行 `npm run install:pi-extension`）；僅同步互動式 Pi 工作階段生命週期和工具活動狀態，並保留 Pi 預設 YOLO 行為
...
一般使用者建議直接從 **[GitHub Releases](https://github.com/rullerzhou-afk/nibbo/releases/latest)** 下載最新的預先建置安裝檔：
...
Nibbo 是社群驅動的專案。歡迎提 Bug、提需求、提 PR —— 在 [Issues](https://github.com/tusharv2005/Nibbo/issues) 聊聊或直接送 PR。
