...
<p align="center">
  <img src="assets/hero.gif" alt="Nibbo 애니메이션 데모: 픽셀 크랩이 AI 코딩 에이전트 상태에 맞춰 잠자기, 생각하기, 도구 실행 중 타이핑, 서브에이전트 1개일 때 헤드폰 그루브, 여러 서브에이전트 병렬 작업 중 세 개 공 저글링, 권한 요청 알림, 작업 완료 축하로 실시간 전환합니다. Claude Code, Codex, Cursor, Copilot, Gemini, Antigravity, Qwen, Pi, OpenClaw 등을 지원합니다.">
</p>
...
- **Pi** — `~/.pi/agent/extensions/nibbo`의 전역 extension으로 연동됩니다 (Clawd 시작 시 자동 등록되며, `npm run install:pi-extension`으로 수동 설치 가능). 인터랙티브 Pi 세션의 라이프사이클과 도구 활동 상태만 보고하며, Pi의 기본 YOLO 동작을 유지합니다.
...
일반 사용자는 **[GitHub Releases](https://github.com/rullerzhou-afk/nibbo/releases/latest)** 에서 최신 사전 빌드 설치 파일을 다운로드하는 것을 권장합니다.

- **Windows**: `Nibbo-Setup-<version>-x64.exe` 또는 `Nibbo-Setup-<version>-arm64.exe`
- **macOS**: `.dmg`
...
Nibbo는 커뮤니티 주도 프로젝트입니다. 버그 리포트, 기능 아이디어, PR 모두 환영합니다. [issue](https://github.com/tusharv2005/Nibbo/issues)를 열어 논의하거나 바로 PR을 보내 주세요.
