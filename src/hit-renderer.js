...
// --- Mouse wheel event (Paper unroll reaction) ---
area.addEventListener("wheel", (e) => {
  if (miniMode || dndEnabled || isReacting || isDragging || isDragReacting) return;
  const sweepingSvg = tc.sweepingSvg || null;
  if (sweepingSvg) {
    playReaction(sweepingSvg, 2000);
  }
});
