...
function Write-NibboFocusResult([string]$token, [string]$reason, [IntPtr]$targetHwnd, [IntPtr]$foregroundHwnd, [bool]$confirmed) {
    if (-not $token) { $token = '' }
...
function makeFocusCmd(sourcePid, cwdCandidates, focusCacheKey = null, wtHwnd = null, focusToken = "") {
  ...
  const parentWindowBlock = psNames ? `
        if ($wtProcessNames -contains $proc.ProcessName) {
            ...
            Save-NibboFocusCache $proc.MainWindowHandle
            $focused = $true
  ...
  const wtTitleMatch = psNames ? `
    ...
    if ($wtMatches.Count -eq 1) {
        ...
        Save-NibboFocusCache $wtMatches[0]
        $focused = $true
        $reason = 'wt-title-match'
    } elseif ($wtMatches.Count -gt 1) {
        $reason = 'wt-title-ambiguous'
    } else {
        $pidWindows = @(Get-NibboVisiblePidWindows -pids $chainWindowsTerminalPids)
        if ($pidWindows.Count -eq 1) {
            [WinFocus]::Focus($pidWindows[0])
            $selectedTargetHwnd = $pidWindows[0]
            Save-NibboFocusCache $pidWindows[0]
...
if ($null -eq $global:NibboFocusWindowCache) {
    $global:NibboFocusWindowCache = @{}
}
function Save-NibboFocusCache([IntPtr]$hwnd) {
    if (-not $focusCacheKey -or $hwnd -eq [IntPtr]::Zero) { return }
    $global:NibboFocusWindowCache[$focusCacheKey] = $hwnd.ToInt64()
}
function Get-NibboCachedWindow() {
    if (-not $focusCacheKey) { return [IntPtr]::Zero }
    if (-not $global:NibboFocusWindowCache.ContainsKey($focusCacheKey)) { return [IntPtr]::Zero }
    try {
        $hwnd = [IntPtr]([int64]$global:NibboFocusWindowCache[$focusCacheKey])
    } catch {
        $global:NibboFocusWindowCache.Remove($focusCacheKey)
        return [IntPtr]::Zero
    }
...
function Get-NibboVisiblePidWindows([int[]]$pids) {
    $windows = @()
...
function Get-NibboWindowsTerminalWindows() {
    $wtPids = @()
    foreach ($wtName in $wtProcessNames) {
    ...
    return @(Get-NibboVisiblePidWindows -pids $wtPids)
}
$curPid = ${sourcePid}
$focused = $false
...
$cachedHwnd = Get-NibboCachedWindow
...
if (-not $focused -and $wtHwndFromHook -ne [IntPtr]::Zero) {
    if ([WinFocus]::IsUsableWindowsTerminalWindow($wtHwndFromHook)) {
        ...
        Save-NibboFocusCache $wtHwndFromHook
        $focused = $true
...
if (-not $focused -and $pendingConsoleHwnd -ne [IntPtr]::Zero) {
    if ($reason -eq 'no-parent-window' -or
        ...
        Save-NibboFocusCache $pendingConsoleHwnd
        $focused = $true
...
Write-NibboFocusResult $focusToken $reason $selectedTargetHwnd $foregroundHwnd $confirmed
`;
}
