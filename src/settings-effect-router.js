...
function createSettingsEffectRouter(options = {}) {
  ...
  function handleSettingsChange({ changes } = {}) {
    ...
    if ("showTray" in changes) {
      safeCall(
        logWarn,
        "Nibbo: tray toggle failed:",
        changes.showTray ? createTray : destroyTray
      );
    }
    if ("showDock" in changes) {
      safeCall(logWarn, "Nibbo: applyDockVisibility failed:", applyDockVisibility);
    }
    ...
    if ("keepAwakeWhileWorking" in changes) {
      safeCall(logWarn, "Nibbo: reconcilePowerSaveBlocker failed:", reconcilePowerSaveBlocker);
    }
    if ("lang" in changes) {
      safeCall(logWarn, "Nibbo: dashboard lang broadcast failed:", sendDashboardI18n);
      safeCall(logWarn, "Nibbo: session HUD lang broadcast failed:", sendSessionHudI18n);
    }
    if ("sessionAliases" in changes) {
      safeCall(
        logWarn,
        "Nibbo: session alias snapshot broadcast failed:",
        emitSessionSnapshot,
        { force: true }
      );
    }
    ...
    if ("hideBubbles" in changes || "permissionBubblesEnabled" in changes) {
      safeCall(logWarn, "Nibbo: syncPermissionShortcuts failed:", syncPermissionShortcuts);
    }
    if (
      ...
      safeCall(
        logWarn,
        "Nibbo: dismiss interactive bubbles failed:",
    ...
    if (
      ...
      try {
        ...
        warn(logWarn, "Nibbo: clear notification bubbles failed:", err);
      }
    } else if (
      ...
      safeCall(
        logWarn,
        "Nibbo: refresh notification bubble timers failed:",
    ...
    if (
      ...
      safeCall(logWarn, "Nibbo: hide update bubble failed:", hideUpdateBubbleForPolicy);
    } else if (
      ...
      safeCall(
        logWarn,
        "Nibbo: refresh update bubble timer failed:",
    ...
    if ("permissionBubbleAutoCloseSeconds" in changes) {
      safeCall(
        logWarn,
        "Nibbo: refresh permission bubble timer failed:",
        refreshPermissionAutoCloseForPolicy
      );
    }
    if ("bubbleFollowPet" in changes) {
      safeCall(logWarn, "Nibbo: repositionFloatingBubbles failed:", repositionFloatingBubbles);
    }
    if ("sessionHudPinned" in changes) {
      ...
      try {
        ...
        warn(logWarn, "Nibbo: session HUD pinned change failed:", err);
      }
    }
    if (
      ...
      try {
        ...
        warn(logWarn, "Nibbo: session HUD setting sync failed:", err);
      }
    }
    if ("sessionHudCleanupDetached" in changes && changes.sessionHudCleanupDetached === true) {
      try {
        ...
        warn(logWarn, "Nibbo: detached session cleanup sweep failed:", err);
      }
    } else if ("sessionHudCleanupDetached" in changes) {
      safeCall(
        logWarn,
        "Nibbo: detached session cleanup snapshot refresh failed:",
        emitSessionSnapshot,
        { force: true }
      );
    }
    if (
      ...
      try {
        ...
        warn(logWarn, "Nibbo: stale cleanup config refresh failed:", err);
      }
    }
    if ("allowEdgePinning" in changes) {
      safeCall(
        logWarn,
        "Nibbo: allowEdgePinning re-clamp failed:",
        reclampPetAfterEdgePinningChange
      );
    }
    if ("disableMiniMode" in changes && changes.disableMiniMode && getMiniMode()) {
      safeCall(logWarn, "Nibbo: disableMiniMode exit failed:", exitMiniMode);
    }
    ...
    for (const key of Object.keys(changes)) {
      if (MENU_AFFECTING_KEYS.has(key)) {
        safeCall(logWarn, "Nibbo: rebuildAllMenus failed:", rebuildAllMenus);
        break;
      }
    }
    ...
    try {
      ...
      warn(logWarn, "Nibbo: settings-changed broadcast failed:", err);
    }
  }
