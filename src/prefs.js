...
// for `nibbo-prefs.json`. It has zero dependencies on Electron, the store, the
...
//
// Bad-file handling: read failure → backup as `nibbo-prefs.json.bak` → return defaults.
...
const SCHEMA = {
  ...
  // Theme
  theme: { type: "string", default: "dewdrop" },
  ...
  agents: {
    ...
    defaultFactory: () => ({
      ...
      // Antigravity is state-only post-D2 — Nibbo never surfaces a permission
  ...
  // Phase 3b-swap: per-theme variant selection (e.g. {nibbo: "chill", calico: "default"}).
...
// nibbo-prefs.json, clamp workingStaleMs down to sessionStaleMs at load time
...
// v3 → v4: Pi returns to a state-only integration. Nibbo no longer inserts a
...
function load(prefsPath) {
  ...
  try {
    ...
    try {
      ...
      console.warn(`Nibbo: prefs file unreadable, backed up to ${bak}:`, err.message);
    } catch (bakErr) {
      console.warn("Nibbo: prefs file unreadable and backup failed:", err.message, bakErr.message);
    }
    return { snapshot: getDefaults(), locked: false };
  }
  ...
  if (incomingVersion > CURRENT_VERSION) {
    console.warn(
      `Nibbo: prefs file version ${incomingVersion} is newer than supported (${CURRENT_VERSION}). ` +
