...
const NIBBO_PROTOCOL_SCHEME = "nibbo";
...
function extractNibboProtocolUrls(argv) {
  if (!Array.isArray(argv)) return [];
  return argv.filter((arg) => typeof arg === "string" && arg.toLowerCase().startsWith(`${NIBBO_PROTOCOL_SCHEME}:`));
}
...
function createCodexPetMain(options = {}) {
  ...
  function getActiveThemeId() {
    ...
    return activeTheme ? activeTheme._id : (settingsController.get("theme") || "dewdrop");
  }
  ...
  function rebuildMenusBestEffort(optionsForRebuild = {}) {
    ...
    try {
      ...
      console.warn("Nibbo: rebuildAllMenus after Codex Pet refresh failed:", err && err.message);
    }
  }
  ...
  function syncThemes(activeThemeId) {
    try {
      ...
      lastSyncSummary = summary;
      console.warn("Nibbo: failed to sync Codex Pet themes:", err && err.message);
      return summary;
    }
  }
  ...
  async function refreshFromSettings() {
    ...
    if (summaryHasActiveCodexPetOrphan(summary, activeId)) {
      const result = await settingsController.applyCommand("setThemeSelection", { themeId: "dewdrop" });
      if (!result || result.status !== "ok") {
        return {
          status: "error",
          message: (result && result.message) || "failed to switch active orphan Codex Pet theme back to dewdrop",
          summary,
        };
      }
      switchedToFallback = true;
      const cleanup = syncThemes("dewdrop");
  ...
  function enqueueImportUrlsFromArgv(argv) {
    for (const rawUrl of extractNibboProtocolUrls(argv)) {
  ...
  function registerProtocolClient() {
    try {
      if (app.isPackaged) {
        return app.setAsDefaultProtocolClient(NIBBO_PROTOCOL_SCHEME);
      }
      if (process.argv.includes(REGISTER_PROTOCOL_DEV_ARG) || process.env.NIBBO_REGISTER_PROTOCOL_DEV === "1") {
        const appRoot = path.resolve(__dirname, "..");
        return app.setAsDefaultProtocolClient(NIBBO_PROTOCOL_SCHEME, process.execPath, [appRoot]);
      }
    } catch (err) {
      console.warn("Nibbo: failed to register nibbo:// protocol:", err && err.message);
    }
    return false;
  }
  ...
  function getImportDialogStrings() {
    const all = {
      en: {
        ...
        confirmDetail: (url) => `Nibbo will download, validate, and install this pet package before switching to it.\n\n${url}`,
      ...
      zh: {
        ...
        confirmDetail: (url) => `Nibbo 会先下载、校验并安装这个宠物包，然后切换到它。\n\n${url}`,
      ...
      "zh-TW": {
        ...
        confirmDetail: (url) => `Nibbo 會先下載、驗證並安裝這個寵物套件，然後切換到它。\n\n${url}`,
      ...
      ko: {
        ...
        confirmDetail: (url) => `Nibbo가 이 펫 패키지를 다운로드, 검증, 설치한 뒤 전환합니다.\n\n${url}`,
      ...
      ja: {
        ...
        confirmDetail: (url) => `Nibbo はこのペットパッケージをダウンロード、検証、インストールしてから切り替えます。\n\n${url}`,
  ...
  async function confirmReplaceExistingPackage(payload) {
    ...
    try {
      ...
      console.warn("Nibbo: Codex Pet replace confirmation failed:", err && err.message);
      return false;
    }
  }
  ...
  function getRemovalDialogStrings() {
    const all = {
      en: {
        ...
        detail: "Nibbo will remove the source package from your Codex pets folder and clean up the generated theme. This cannot be undone.",
      },
      zh: {
        uninstall: "卸载",
        cancel: "取消",
        message: (name) => `卸载导入宠物 "${name}"？`,
        detail: "Nibbo 会从 Codex pets 文件夹删除源包，并清理生成的主题。此操作不可撤销。",
      },
      "zh-TW": {
        uninstall: "解除安裝",
        cancel: "取消",
        message: (name) => `解除安裝匯入的寵物「${name}」？`,
        detail: "Nibbo 會從 Codex pets 資料夾移除來源套件，並清理產生的主題。此操作無法復原。",
      },
      ko: {
        uninstall: "제거",
        cancel: "취소",
        message: (name) => `가져온 펫 "${name}"을(를) 제거할까요?`,
        detail: "Nibbo가 Codex pets 폴더의 원본 패키지를 제거하고 생성된 테마를 정리합니다. 이 작업은 되돌릴 수 없습니다.",
      },
      ja: {
        uninstall: "アンインストール",
        cancel: "キャンセル",
        message: (name) => `インポート済みペット "${name}" をアンインストールしますか？`,
        detail: "Nibbo は Codex pets フォルダから元パッケージを削除し、生成されたテーマをクリーンアップします。この操作は元に戻せません。",
      },
    };
    return all[getLang()] || all.en;
  }
  ...
  async function confirmRemoveImportedPackage(displayName) {
    ...
    try {
      ...
      console.warn("Nibbo: Codex Pet removal confirmation failed:", err && err.message);
      return false;
    }
  }
  ...
  async function materializeAndActivateImportedPet(imported) {
    ...
    if (!generated || !generated.themeId) {
      throw new Error("imported package did not materialize into a Nibbo theme");
    }
  ...
  async function handleImportProtocolUrl(rawUrl) {
    let parsed;
    try {
      parsed = codexPetImporter.parseNibboImportUrl(rawUrl);
    } catch (err) {
    ...
    try {
      ...
      console.warn("Nibbo: Codex Pet import confirmation failed:", err && err.message);
      return;
    }
    ...
    try {
      ...
      console.warn("Nibbo: Codex Pet import failed:", err && err.message);
      await showImportError(err && err.message);
    }
  }
  ...
  async function openCodexPetsDir() {
    try {
      ...
      console.warn("Nibbo: settings:open-codex-pets-dir failed:", err && err.message);
  ...
  async function importCodexPetZip(event) {
    ...
    try {
      ...
      console.warn("Nibbo: Codex Pet zip picker failed:", err && err.message);
      return { status: "error", message: (err && err.message) || String(err) };
    }
    ...
    try {
      ...
      console.warn("Nibbo: Codex Pet zip import failed:", err && err.message);
      return { status: "error", message: (err && err.message) || String(err) };
    }
  }
  ...
  async function removeCodexPet(themeId) {
    ...
    try {
      ...
      console.warn("Nibbo: Codex Pet removal failed:", err && err.message);
      return { status: "error", message: (err && err.message) || String(err) };
    }
  }
...
module.exports.NIBBO_PROTOCOL_SCHEME = NIBBO_PROTOCOL_SCHEME;
