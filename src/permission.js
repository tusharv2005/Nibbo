...
function deferMacFloatingVisibility(ctx, win) {
  ...
  win.__nibboMacDeferredVisibilityUntil = deferUntil;
  setTimeout(() => {
    if (!win || win.isDestroyed()) return;
    if (win.__nibboMacDeferredVisibilityUntil === deferUntil) {
      delete win.__nibboMacDeferredVisibilityUntil;
    }
...
//
// followPet=false → bottom-right of the work area (default Nibbo behavior).
//
...
function showPermissionBubble(permEntry) {
  ...
  // Nibbo on some setups. Defer topmost restoration until after showInactive.
...
// Telegram-only "Tool input hidden by Nibbo." card would let the user approve
// a black box.
...
function buildRemoteApprovalPayload(permEntry) {
  ...
  // Label is "Folder" (not "Session") on purpose: the pinned cc-connect-nibbo
...
function sendPermissionResponse(res, decisionOrBehavior, message, hookEventName = "PermissionRequest") {
  ...
  res.writeHead(200, {
    "Content-Type": "application/json",
    [NIBBO_SERVER_HEADER]: CLAWD_SERVER_ID,
  });
...
function sendNoDecisionResponse(res, reason = "", label = "permission") {
  ...
  res.writeHead(204, { [NIBBO_SERVER_HEADER]: CLAWD_SERVER_ID });
  res.end();
  return true;
}
...
function cleanup() {
  ...
  for (const perm of [...pendingPermissions]) {
    ...
    if (perm.isCodex || perm.isQwenCode || perm.isCopilotCli || perm.isAntigravity || perm.isHermes) resolvePermissionEntry(perm, "no-decision", "Nibbo is quitting");
    else resolvePermissionEntry(perm, "deny", "Nibbo is quitting");
  }
}
