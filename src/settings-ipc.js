...
function registerSettingsIpc(options = {}) {
  ...
  const aboutHeroSvgPath = options.aboutHeroSvgPath
    || path.join(__dirname, "..", "assets", "svg", "nibbo-about-hero.svg");
  const disposers = [];
  ...
  handle("settings:list-themes", () => {
    try {
      const activeTheme = getActiveTheme();
      const activeId = activeTheme ? activeTheme._id : "nibbo";
      ...
      console.warn("Nibbo: settings:list-themes failed:", err && err.message);
      return [];
    }
  });
  ...
  handle("settings:import-user-theme-zip", async (event) => {
    ...
    try {
      result = await dialog.showOpenDialog(getDialogParent(event), {
        properties: ["openFile"],
        filters: [{ name: "Nibbo theme zip", extensions: ["zip"] }],
      });
    } catch (err) {
  ...
  handle("settings:confirm-remove-theme", async (event, themeId) => {
    ...
    try {
      ...
      console.warn("Nibbo: confirm-remove-theme dialog failed:", err && err.message);
  ...
  handle("settings:list-agents", () => {
    try {
      ...
      console.warn("Nibbo: settings:list-agents failed:", err && err.message);
      return [];
    }
  });
  ...
  handle("settings:get-about-info", () => {
    ...
    try {
      ...
      console.warn("Nibbo: failed to read about hero SVG:", err && err.message);
    }
    ...
    return {
      ...
      repoUrl: "https://github.com/tusharv2005/Nibbo",
      license: "AGPL-3.0",
      copyright: "\u00a9 2026 tusharv2005",
      authorName: "tusharv2005",
      authorUrl: "https://github.com/tusharv2005",
      heroSvgContent,
