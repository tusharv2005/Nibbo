...
function initWithConfig(cfg) {
  ...
  _idleFollowSvg = tc.idleFollowSvg || "nibbo-idle-follow.svg";
  ...
  applyObjectScaleStyle(nibboEl, getObjectSvgName(nibboEl), null);
...
function getCurrentSvgRoot() {
  if (!nibboEl || nibboEl.tagName !== "OBJECT") return null;
  try {
    const svgDoc = nibboEl.contentDocument;
...
function setCurrentScriptedSvgLowPowerPaused(paused) {
  const target = nibboEl;
  if (!target || target.tagName !== "OBJECT") return;
  try {
    const fn = target.contentWindow && target.contentWindow.__nibboSetLowPowerPaused;
...
function setViewportOffset(offsetY) {
  ...
  applyObjectScaleStyle(nibboEl, currentDisplayedSvg, currentState);
  if (pendingNext) {
...
window.electronAPI.onMiniModeChange((enabled, edge, options) => {
  ...
  applyMiniFlip(nibboEl, currentState);
  if (miniLeftFlip) {
    applyGlyphFlipCompensation(nibboEl);
  } else {
    removeGlyphFlipCompensation(nibboEl);
  }
...
function applyGlyphFlipCompensation(objectEl) {
  ...
  try {
    ...
    if (svgWindow && typeof svgWindow.__nibboSetGlyphFlipCompensation === "function") {
      svgWindow.__nibboSetGlyphFlipCompensation(true);
    }
...
function removeGlyphFlipCompensation(objectEl) {
  ...
  try {
    ...
    const svgWindow = objectEl.contentWindow;
    if (svgWindow && typeof svgWindow.__nibboSetGlyphFlipCompensation === "function") {
      svgWindow.__nibboSetGlyphFlipCompensation(false);
    }
...
/**
 * Determine if a state should attach Nibbo-controlled eye tracking.
 */
...
function applyCloudlingPointerBridge(payload) {
  ...
  callCloudlingPointerBridge(nibboEl, getDisplayedCloudlingPointerPayload(normalized));
}

function clearCloudlingPointerBridge(objectEl = nibboEl) {
  const payload = {
...
let scrollPaperTimer = null;
window.electronAPI.onGlobalScrollTriggered(() => {
  if (container) {
    container.classList.add("scrolling-paper");
    if (scrollPaperTimer) clearTimeout(scrollPaperTimer);
    scrollPaperTimer = setTimeout(() => {
      container.classList.remove("scrolling-paper");
      scrollPaperTimer = null;
    }, 2000);
  }
});
...
let currentDisplayedSvg = getObjectSvgName(nibboEl);
...
function getPetMediaElements() {
  return [...container.querySelectorAll("object, img.nibbo-img")];
}
...
function scheduleSwapVisibilityRescue(token, file, state) {
  ...
  const timer = setTimeout(() => {
    ...
    if (forceVisiblePetElement(nibboEl)) return;
    forceImageChannelReload(file, state);
...
function forceImageChannelReload(file, state, allowImageFallback = true) {
  ...
  console.warn("Nibbo: animation stayed invisible; reloading through the image channel:", file);
...
function swapToFile(file, state, useObjectChannel, options = {}) {
  ...
  if (useObj) {
    ...
    const swap = () => {
      ...
      for (const child of [...container.querySelectorAll("object, img.nibbo-img")]) {
        if (child !== next) {
...
window.electronAPI.onStateChange((state, svg, activeCount) => {
  ...
  if (container) {
    if (typeof activeCount === "number" && activeCount >= 3) {
      container.classList.add("overheat");
    } else {
      container.classList.remove("overheat");
    }
  }
  ...
  const alreadyDisplayed = nibboEl && nibboEl.isConnected
    && currentDisplayedSvg === svg
    && currentDisplayedAssetUrl === desiredAssetUrl;
  const displayedChannelMatches = !alreadyDisplayed || ((nibboEl.tagName === "OBJECT") === desiredObjectChannel);
  ...
  if ((alreadyDisplayed && displayedChannelMatches) || (alreadyPending && pendingChannelMatches)) {
    if (alreadyDisplayed) {
      if (needsEyeTracking(state) && !eyeTarget && !_trackingLayers) {
        if (nibboEl.tagName === "OBJECT") attachEyeTracking(nibboEl);
...
window.electronAPI.onKimiPermissionPulse(() => {
  if (nibboEl && nibboEl.isConnected && currentDisplayedSvg) {
...
//      Used by default nibbo theme (tc.eyeTracking.ids config)
...
function _cleanupLayeredTracking() {
  ...
  if (_trackingLayers && nibboEl && nibboEl.tagName === "OBJECT") {
    try {
      _unwrapAll(nibboEl.contentDocument);
    } catch {}
  }
...
window.electronAPI.onEyeMove((dx, dy) => {
  ...
  if (eyeTarget && !eyeTarget.ownerDocument?.defaultView) {
    ...
    bodyTarget = null;
    shadowTarget = null;
    if (nibboEl && nibboEl.isConnected && nibboEl.tagName === "OBJECT") attachEyeTracking(nibboEl);
    return;
  }
...
function reportSoundPlaybackError(phase, err) {
  ...
  try { console.warn(`Nibbo sound ${phase} failed:`, message); } catch {}
}
...
function playSynthMeow() {
  try {
    ...
    // Vocal contour: me-ow (450Hz -> 750Hz -> 400Hz)
    ...
    osc2.frequency.exponentialRampToValueAtTime(758, now + 0.08);
    osc2.frequency.exponentialRampToValueAtTime(404, now + 0.3);
...
window.electronAPI.onWakeFromDoze(() => {
  if (nibboEl && nibboEl.tagName === "OBJECT" && nibboEl.contentDocument) {
    try {
      const eyes = nibboEl.contentDocument.getElementById(_eyeIds.dozeEyes || "eyes-doze");
