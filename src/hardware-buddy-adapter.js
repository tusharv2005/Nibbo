...
function wrapHardwareBuddyTransport(transport) {
  if (!transport || typeof transport.send !== "function" || transport.__nibboSanitizedSend === true) return transport;
  ...
  Object.defineProperty(transport, "__nibboSanitizedSend", {
    value: true,
    enumerable: false,
  });
  return transport;
...
function loadCoreModules(coreRoot) {
  ...
  if (missingPaths.length) {
    const err = new Error(
      "Hardware Buddy core modules are missing. Clone https://github.com/tusharv2005/clawstick next to this repo or set CLAWD_HARDWARE_BUDDY_ROOT."
    );
...
function classifyHardwareBuddyIssue(err) {
  ...
  if (code === "CORE_MISSING") {
    return {
      ...
      hint: "Clone https://github.com/tusharv2005/clawstick next to this repo or set CLAWD_HARDWARE_BUDDY_ROOT.",
    };
  }
