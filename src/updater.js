...
const RELEASES_LATEST_URL = "https://github.com/tusharv2005/Nibbo/releases/latest";
...
function initUpdater(ctx, deps = {}) {
  ...
  // single-writer architecture and persist to nibbo-prefs.json.
  ...
  async function showUpToDateBubble(version) {
    ...
    return showInfoBubble(
      ...
      t("updateNotAvailableMsg", "Nibbo v{version} is the latest version.").replace("{version}", version),
      {
        version,
  ...
  function fetchLatestReleaseViaRedirect() {
    return new Promise((resolve, reject) => {
      const req = httpsGet({
        hostname: "github.com",
        path: "/tusharv2005/Nibbo/releases/latest",
        headers: {
          "User-Agent": "Nibbo",
          Accept: "text/html,*/*",
        },
  ...
  function fetchLatestReleaseFromApi() {
    return new Promise((resolve, reject) => {
      const headers = { "User-Agent": "Nibbo" };
      if (lastReleaseEtag) headers["If-None-Match"] = lastReleaseEtag;
      const req = httpsGet({
        hostname: "api.github.com",
        path: "/repos/tusharv2005/Nibbo/releases/latest",
        headers,
      }, (res) => {
  ...
  async function maybePromptNativeArm64Installer(release, { manual = false, currentVersion = app.getVersion() } = {}) {
    ...
    const action = await awaitBubbleAction(showBubble({
      ...
      message: t(
        ...
        "Nibbo v{version} has a native Windows ARM64 installer. Install it for better performance and battery life?"
  ...
  async function runGitUpdate(repoRoot, branch, localHead) {
    ...
    await showSuccessBubble({
      title: t("updateReady", "Update Ready"),
      message: t("gitUpdateRestarting", "Update complete. Restarting Nibbo now..."),
    });
    ...
    if (!autoUpdater) {
      ...
      if (manual) {
        await showErrorBubble({
          ...
          nextStep: "Restart Nibbo or reinstall the packaged app, then try again.",
