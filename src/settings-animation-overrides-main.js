...
const ANIMATION_OVERRIDES_EXPORT_DIALOG_STRINGS = {
  en: {
    ...
    defaultName: (ts) => `nibbo-animation-overrides-${ts}.json`,
    jsonFilter: "Nibbo Animation Overrides",
...
function createSettingsAnimationOverridesMain(options = {}) {
  ...
  function runPendingPostReloadTasks() {
    ...
    for (const task of tasks) {
      try { task(); } catch (err) { console.warn("Nibbo: post-reload task threw:", err && err.message); }
    }
  }
  ...
  async function runAnimationPreviewPosterJob(job) {
    try {
      ...
      if (err && err.code === "ANIMATION_PREVIEW_POSTER_TIMEOUT") {
        console.warn("Nibbo: animation preview poster capture timed out:", message);
      } else {
        console.warn("Nibbo: failed to capture animation preview poster:", message);
      }
    } finally {
  ...
  async function exportAnimationOverrides(event) {
    ...
    try {
      ...
      const payload = {
        nibboAnimationOverrides: ANIMATION_OVERRIDES_EXPORT_VERSION,
        version: ANIMATION_OVERRIDES_EXPORT_VERSION,
        exportedAt: new Date().toISOString(),
        nibboVersion: app.getVersion(),
        themes: overrides,
      };
      ...
      console.warn("Nibbo: export-animation-overrides failed:", err && err.message);
  ...
  async function importAnimationOverrides(event) {
    ...
    try {
      ...
      console.warn("Nibbo: import-animation-overrides dialog failed:", err && err.message);
    ...
    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
      return { status: "error", message: "file is not a Nibbo animation overrides export" };
    }
    const magic = parsed.nibboAnimationOverrides;
