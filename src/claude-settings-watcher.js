...
const HOOK_MARKER = "nibbo-hook.js";
 ...
 * TODO: Decide whether non-Nibbo HTTP hooks should contribute to third-party shrink detection.
...
function createClaudeSettingsWatcher(ctx = {}) {
  ...
  function start() {
    ...
    // so the very first watcher event after Nibbo boots (e.g. an external CLI minimize
    // landing right after syncNibboHooks() ran) can be compared against a real snapshot
    ...
    try {
      ...
      console.warn("Nibbo: could not seed settings baseline:", err.message);
    }
    try {
          ...
          try {
            ...
            if (settingsNeedClaudeHookResync(raw, expectedPermissionUrl)) {
              ...
              if (isSuspiciousShrink(lastTrustedSnapshot, currentSnapshot, suspiciousShrinkRatio, suspiciousKeyLossThreshold)) {
                console.warn("Nibbo: settings.json shrank suspiciously — skipping auto-resync to preserve third-party hooks");
                if (typeof ctx.notifySuspiciousShrink === "function") {
                  ctx.notifySuspiciousShrink(lastTrustedSnapshot, currentSnapshot);
                }
                return;
              }
              console.log("Nibbo: hooks missing from settings.json — re-registering");
              settingsWatchLastSyncTime = nowFn();
              if (typeof ctx.syncNibboHooks === "function") ctx.syncNibboHooks();
      ...
      if (settingsWatcher && typeof settingsWatcher.on === "function") settingsWatcher.on("error", (err) => {
        console.warn("Nibbo: settings watcher error:", err.message);
      });
      return true;
    } catch (err) {
      console.warn("Nibbo: failed to watch settings directory:", err.message);
      settingsWatcher = null;
      return false;
    }
  }
