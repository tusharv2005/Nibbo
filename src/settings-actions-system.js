...
function restartNibbo(payload, deps) {
  if (!payload || payload.confirmed !== true) {
    return { status: "error", message: "restartNibbo requires confirmation" };
  }
  if (!deps || typeof deps.restartNibbo !== "function") {
    return { status: "error", message: "restartNibbo requires deps.restartNibbo" };
  }
  try {
    deps.restartNibbo();
    return { status: "ok", message: "Nibbo is restarting" };
  } catch (err) {
    return { status: "error", message: `restartNibbo: ${err && err.message}` };
  }
}
...
function createRepairDoctorIssue({ repairAgentIntegration, setBubbleCategoryEnabled } = {}) {
  return async function repairDoctorIssue(payload, deps) {
    ...
    if (type === "restart-nibbo") {
      return restartNibbo(payload, deps);
    }
    return {
      status: "error",
