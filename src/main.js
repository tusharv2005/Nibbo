...
if (isWin) {
  try {
    ...
    console.warn("Nibbo: koffi/AllowSetForegroundWindow not available:", err.message);
  }
}

// ── Windows: switch the dev console to UTF-8 ──
//
// `npm start` attaches Nibbo to a parent PowerShell/cmd console. That
...
// stdout/stderr as UTF-8 while Nibbo is running. Packaged builds run under
...
if (isWin) {
  try {
    ...
    console.warn("Nibbo: SetConsoleOutputCP(65001) failed:", err && err.message);
  }
}
...
const PREFS_PATH = path.join(app.getPath("userData"), "nibbo-prefs.json");
...
function _restartNibboNow() {
  if (_restartScheduled) return;
  _restartScheduled = true;
  // Triggered by Doctor's restart-nibbo repair. relaunch() queues a fresh
...
function hydrateSystemBackedSettings() {
  ...
  try {
    ...
    console.warn("Nibbo: failed to read system openAtLogin during hydration:", err && err.message);
  }
  ...
  if (result && result.status === "error") {
    console.warn("Nibbo: openAtLogin hydration failed:", result.message);
  }
}
...
function safeConsoleError(...args) {
  try {
    ...
    try {
      ...
      fs.appendFileSync(path.join(app.getPath("userData"), "nibbo-main.log"), line);
    } catch {}
  }
}
...
// If lenient fell back to "nibbo" OR the variant fell back to "default",
...
let _requestedThemeId = _settingsController.get("theme") || "nibbo";
...
if (codexPetMain.summaryHasActiveOrphan(_startupCodexPetSyncSummary, _requestedThemeId)) {
  ...
  if (result && result.status === "error") {
    console.warn("Nibbo: Codex Pet active theme fallback hydrate failed:", result.message);
  }
...
if (_loadedStartupTheme._id !== _requestedThemeId || _loadedStartupTheme._variantId !== _requestedVariantId) {
  ...
  if (result && result.status === "error") {
    console.warn("Nibbo: theme hydrate after fallback failed:", result.message);
  }
}
...
function reconcilePowerSaveBlocker() {
  try {
    ...
    console.warn("Nibbo: reconcilePowerSaveBlocker failed:", err);
  }
}
...
// a malicious or accidental NIBBO_TG_BOT_TOKEN_FILE / CLAWD_BRIDGE_CONFIG can't
...
async function startTelegramApprovalSidecar() {
  ...
  // The sidecar reads it from there directly — Nibbo's main process must never
  // pipe a token value through process.env or child env, so there is no
  // botToken option here and no NIBBO_TG_BOT_TOKEN read from process.env.
...
async function sendTelegramApprovalTest() {
  ...
  try {
    const decision = await client.requestApproval({
      title: "Nibbo Telegram approval test",
...
function broadcastHardwareBuddyStatus(status) {
  ...
  try {
    ...
    console.warn("Nibbo: Hardware Buddy status broadcast failed:", err && err.message);
  }
}
...
function sendHardwareBuddyTestApproval() {
  ...
  const promise = new Promise((resolve) => {
    ...
    permEntry = {
      ...
      cwd: __dirname,
      codexOriginator: "nibbo-settings",
...
unsubscribeHardwareBuddySettings = _settingsController.subscribeKey("hardwareBuddy", () => {
  ...
  try {
    ...
    console.warn("Nibbo: failed to apply Hardware Buddy settings:", err && err.message);
...
const _menuCtx = {
  ...
  set hideBubbles(v) { _settingsController.applyCommand("setAllBubblesHidden", { hidden: !!v }).catch((err) => {
    console.warn("Nibbo: setAllBubblesHidden failed:", err && err.message);
  }); },
  ...
  getActiveThemeId: () => themeRuntime.getActiveThemeId("nibbo"),
...
// the scheduler start/stop without restarting Nibbo.
try {
...
const _remoteSshRuntime = createRemoteSshRuntime({
  getHookServerPort: () => getHookServerPort(),
  log: (...args) => console.warn("Nibbo remote-ssh:", ...args),
});
...
registerSettingsIpc({
  ...
  checkForUpdates,
  aboutHeroSvgPath: path.join(__dirname, "..", "assets", "svg", "nibbo-about-hero.svg"),
...
registerSessionIpc({
  ...
  setSessionHudPinned: (value) => {
    ...
    if (result && typeof result.then === "function") {
      result
        .then((r) => {
          if (r && r.status === "error") console.warn("Nibbo: failed to pin Session HUD:", r.message);
        })
        .catch((err) => console.warn("Nibbo: failed to pin Session HUD:", err && err.message));
    } else if (result && result.status === "error") {
      console.warn("Nibbo: failed to pin Session HUD:", result.message);
    }
  },
  getLanWsServer: () => _lanWss,
});
...
const EXT_ID = "nibbo.nibbo-terminal-focus";
...
function installTerminalFocusExtension() {
  ...
  if (!fs.existsSync(extSrc)) {
    console.log("Nibbo: terminal-focus extension source not found, skipping auto-install");
    return;
  }
  ...
  for (const extRoot of targets) {
    ...
    try {
      ...
      installed++;
      console.log(`Nibbo: installed terminal-focus extension to ${dest}`);
    } catch (err) {
      console.warn(`Nibbo: failed to install extension to ${dest}:`, err.message);
    }
  }
  if (installed > 0) {
    console.log(`Nibbo: terminal-focus extension installed to ${installed} editor(s). Restart VS Code/Cursor to activate.`);
  }
}
...
if (!gotTheLock) {
  if (process.argv.includes(REGISTER_PROTOCOL_DEV_ARG)) {
    ...
    console.log(`Nibbo: nibbo:// dev protocol registration ${protocolRegistered ? "succeeded" : "failed"}`);
  }
  ...
  // --- Global Mouse Scroll Event Tap via macOS CoreGraphics FFI ---
  let scrollEventTapPort = null;
  let scrollEventTapSource = null;
  let scrollEventTapCallback = null;
  ...
  function setupGlobalScrollListener() {
    ...
    try {
      ...
      const CGEventTapCallBack = koffi.proto("void * CGEventTapCallBack(void *proxy, uint32_t type, void *event, void *refcon)");
      const CGEventTapCreate = cg.func("void * CGEventTapCreate(uint32_t tap, uint32_t place, uint32_t options, uint64_t eventsOfInterest, CGEventTapCallBack *callback, void *refcon)");
      ...
      scrollEventTapCallback = koffi.register((proxy, type, event, refcon) => {
        try {
          const now = Date.now();
          if (now - lastScrollTime >= SCROLL_COOLDOWN_MS) {
            lastScrollTime = now;
            const themeConfig = themeRuntime.getHitRendererConfig();
            const sweepingSvg = themeConfig && themeConfig.sweepingSvg;
            if (sweepingSvg) {
              sendToRenderer("play-click-reaction", sweepingSvg, 2000);
              sendToRenderer("global-scroll-triggered");
            }
          }
        } catch (e) {
          console.error("Nibbo: Error in scroll event callback:", e);
        }
        return event;
      }, koffi.pointer(CGEventTapCallBack));

      const tap = 1; // kCGSessionEventTap
      const place = 0; // kCGHeadInsertEventTap
      const options = 1; // kCGEventTapOptionListenOnly
      const mask = 1n << 22n; // kCGEventScrollWheel

      scrollEventTapPort = CGEventTapCreate(tap, place, options, mask, scrollEventTapCallback, null);
      if (scrollEventTapPort) {
        scrollEventTapSource = CFMachPortCreateRunLoopSource(null, scrollEventTapPort, 0);
        const rl = CFRunLoopGetCurrent();
        CFRunLoopAddSource(rl, scrollEventTapSource, kCFRunLoopCommonModes);
        console.log("Nibbo: Global scroll listener tap successfully registered.");
      } else {
        console.warn("Nibbo: Failed to create global scroll CGEventTap (likely requires Accessibility permission).");
      }
    } catch (err) {
      console.warn("Nibbo: Failed to setup global scroll listener:", err.message);
    }
  }

  function cleanupGlobalScrollListener() {
    if (process.platform !== 'darwin') return;
    try {
      if (scrollEventTapSource && scrollEventTapPort) {
        const koffi = require("koffi");
        const cf = koffi.load("/System/Library/Frameworks/CoreFoundation.framework/CoreFoundation");
        const CFRunLoopGetCurrent = cf.func("void * CFRunLoopGetCurrent()");
        ...
        CFRelease(scrollEventTapSource);
        CFRelease(scrollEventTapPort);
      }
    } catch (err) {
      console.warn("Nibbo: Failed to remove scroll event tap source/port:", err.message);
    } finally {
      scrollEventTapSource = null;
      scrollEventTapPort = null;
    }

    if (scrollEventTapCallback) {
      try {
        const koffi = require("koffi");
        koffi.unregister(scrollEventTapCallback);
      } catch (_) {}
      scrollEventTapCallback = null;
    }
  }
  ...
  app.whenReady().then(() => {
    ...
    initTelegramMigrationController().catch((err) => {
      console.warn("Nibbo: migration controller init failed:", err && err.message);
    });
    createWindow();
    ...
    codexPetMain.flushPendingImportUrls().catch((err) => {
      console.warn("Nibbo: Codex Pet import queue failed:", err && err.message);
    });
    ...
    try {
      ...
      console.warn("Nibbo: failed to start Hardware Buddy adapter:", err && err.message);
      hardwareBuddyLog(`start failed: ${err && err.message ? err.message : err}`);
    }

    // Auto-install VS Code/Cursor terminal-focus extension
    try { installTerminalFocusExtension(); } catch (err) {
      console.warn("Nibbo: failed to auto-install terminal-focus extension:", err.message);
    }
    ...
    // Start global scroll monitoring
