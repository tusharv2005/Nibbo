...
async function deploy({ profile, runtime, deps = {} }) {
  ...
  if (typeof profile.hostPrefix === "string" && profile.hostPrefix.length > 0) {
    ...
    const args = buildSshArgs(profile).concat([
      "cat > ~/.claude/hooks/nibbo-host-prefix",
    ]);
...
//
// `~/.nibbo-codex-monitor.pid` on the remote is a fixed marker holding the
...
async function startCodexMonitor({ profile, runtime = null, deps = {} }) {
  ...
  const cleanCmd =
    "[ -f ~/.nibbo-codex-monitor.pid ] && kill $(cat ~/.nibbo-codex-monitor.pid) 2>/dev/null; " +
    "rm -f ~/.nibbo-codex-monitor.pid; true";
  ...
  const startCmd =
    ...
    "> /dev/null 2>&1 & echo $! > ~/.nibbo-codex-monitor.pid";
