...
function delegateWindowToStationarySpace(nsWindow) {
  try {
    ...
    if (!warnedSkyLightFailure) {
      console.warn("Nibbo: failed to move macOS window into stationary SkyLight space:", err.message);
...
function applyStationaryCollectionBehavior(browserWindow) {
  ...
  try {
    ...
    if (!warnedApplyFailure) {
      console.warn("Nibbo: failed to apply macOS stationary window behavior:", err.message);
