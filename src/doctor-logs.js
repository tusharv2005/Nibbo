...
function resolveNibboLogTarget(options = {}) {
  ...
  const fallbackDir = dirs[0] || pathApi.join(os.homedir(), ".nibbo");
  return { status: "directory", path: fallbackDir, reason: "no-log-found" };
}

async function openNibboLog(options = {}) {
  const fsApi = options.fs || fs;
  const shell = options.shell;
  if (!shell || typeof shell.openPath !== "function") {
    return { status: "error", reason: "shell-unavailable" };
  }
  const target = resolveNibboLogTarget(options);
