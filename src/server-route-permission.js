...
function handlePermissionPost(req, res, options) {
  ...
  req.on("end", () => {
    if (tooLarge) {
      ...
      ctx.sendPermissionResponse(res, "deny", "Permission request too large for Nibbo bubble; answer in terminal");
      return;
    }
    ...
    try {
      ...
      if (agentId === "opencode") {
        res.writeHead(200, { [NIBBO_SERVER_HEADER]: CLAWD_SERVER_ID });
        res.end("ok");
        ...
        // channel Nibbo uses to send the decision back to the plugin,
      ...
      // Nibbo intentionally does NOT show a permission bubble for agy. If a
      ...
      // permission prompt". Keep every fallback as 204/no-decision so Nibbo
      ...
      // continue native flow" (Phase 0 §3, locked). Every Nibbo path here
      ...
      // Pi is state-only in Nibbo. Current extensions never POST /permission.
      ...
      // instead of turning Nibbo fallback into a terminal confirmation prompt.
      if (agentId === "pi") {
        const toolName = typeof data.tool_name === "string" && data.tool_name ? data.tool_name : "unknown";
        if (ctx.doNotDisturb) {
          recordRequestHookEvent.droppedByDnd();
      ...
      // Tag the permEntry with the source agent. Nibbo's HTTP permission
