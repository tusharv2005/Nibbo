...
function createIntegrationSyncRuntime(options = {}) {
  ...
  function syncNibboHooks() {
    try {
      if (typeof ctx.syncNibboHooksImpl === "function") {
        return ctx.syncNibboHooksImpl({
      ...
      if (added > 0 || updated > 0 || removed > 0) {
        console.log(`Nibbo: synced hooks (added ${added}, updated ${updated}, removed ${removed})`);
      }
      return { status: "ok", added, updated, removed };
    } catch (err) {
      console.warn("Nibbo: failed to sync hooks:", err.message);
  ...
  function syncGeminiHooks() {
    try {
      ...
      if (added > 0 || updated > 0) {
        console.log(`Nibbo: synced Gemini hooks (added ${added}, updated ${updated})`);
      }
      return { status: "ok", added, updated };
    } catch (err) {
      console.warn("Nibbo: failed to sync Gemini hooks:", err.message);
  ...
  function syncAntigravityHooks() {
    try {
      ...
      if (result.added > 0 || result.updated > 0) {
        console.log(`Nibbo: synced Antigravity hooks (added ${result.added}, updated ${result.updated})`);
      }
      return { status: "ok", ...result };
    } catch (err) {
      console.warn("Nibbo: failed to sync Antigravity hooks:", err.message);
  ...
  function syncCodeBuddyHooks() {
    try {
      ...
      if (added > 0 || updated > 0) {
        console.log(`Nibbo: synced CodeBuddy hooks (added ${added}, updated ${updated})`);
      }
      return { status: "ok", added, updated };
    } catch (err) {
      console.warn("Nibbo: failed to sync CodeBuddy hooks:", err.message);
  ...
  function syncKiroHooks() {
    try {
      ...
      if (added > 0 || updated > 0) {
        console.log(`Nibbo: synced Kiro hooks (added ${added}, updated ${updated})`);
      }
      return { status: "ok", added, updated };
    } catch (err) {
      console.warn("Nibbo: failed to sync Kiro hooks:", err.message);
  ...
  function syncKimiHooks() {
    try {
      ...
      if (added > 0 || updated > 0) {
        console.log(`Nibbo: synced Kimi hooks (added ${added}, updated ${updated})`);
      }
      return { status: "ok", added, updated };
    } catch (err) {
      console.warn("Nibbo: failed to sync Kimi hooks:", err.message);
  ...
  function syncQwenHooks() {
    try {
      ...
      if (result.added > 0 || result.updated > 0) {
        console.log(`Nibbo: synced Qwen hooks (added ${result.added}, updated ${result.updated})`);
      }
      return { status: "ok", ...result };
    } catch (err) {
      console.warn("Nibbo: failed to sync Qwen hooks:", err.message);
  ...
  function syncCodexHooks() {
    try {
      ...
      if (added > 0 || updated > 0) {
        console.log(`Nibbo: synced Codex hooks (added ${added}, updated ${updated})`);
      }
      if (Array.isArray(warnings)) {
        for (const warning of warnings) console.warn(`Nibbo: Codex hook sync warning: ${warning}`);
      }
      return { status: "ok", added, updated, warnings };
    } catch (err) {
      console.warn("Nibbo: failed to sync Codex hooks:", err.message);
  ...
  function repairCodexHooks(options = {}) {
    try {
      ...
      if (added > 0 || updated > 0 || configChanged) {
        console.log(`Nibbo: repaired Codex hooks (added ${added}, updated ${updated}, configChanged=${!!configChanged})`);
      }
      if (Array.isArray(warnings)) {
        for (const warning of warnings) console.warn(`Nibbo: Codex hook repair warning: ${warning}`);
      ...
      console.warn("Nibbo: failed to repair Codex hooks:", err.message);
  ...
  function syncCursorHooks() {
    try {
      ...
      if (added > 0 || updated > 0) {
        console.log(`Nibbo: synced Cursor hooks (added ${added}, updated ${updated})`);
      }
      return { status: "ok", added, updated };
    } catch (err) {
      console.warn("Nibbo: failed to sync Cursor hooks:", err.message);
  ...
  function syncCopilotHooks() {
    try {
      ...
      if (added > 0 || updated > 0) {
        console.log(`Nibbo: synced Copilot hooks (added ${added}, updated ${updated})`);
      }
      return { status: "ok", added, updated };
    } catch (err) {
      console.warn("Nibbo: failed to sync Copilot hooks:", err.message);
  ...
  function syncOpencodePlugin() {
    try {
      ...
      if (added || created) {
        console.log(`Nibbo: synced opencode plugin (added=${added}, created=${created})`);
      }
      return { status: "ok", added, created };
    } catch (err) {
      console.warn("Nibbo: failed to sync opencode plugin:", err.message);
  ...
  function syncPiExtension() {
    try {
      ...
      if (result.installed && result.updated) {
        console.log("Nibbo: synced Pi extension");
      }
      return { status: "ok", ...result };
    } catch (err) {
      console.warn("Nibbo: failed to sync Pi extension:", err.message);
  ...
  function syncOpenClawPlugin() {
    try {
      ...
      if (result.installed && result.updated) {
        console.log("Nibbo: synced OpenClaw plugin");
      }
      return { status: "ok", ...result };
    } catch (err) {
      console.warn("Nibbo: failed to sync OpenClaw plugin:", err.message);
  ...
  function repairOpenClawPlugin() {
    try {
      ...
      console.warn("Nibbo: failed to repair OpenClaw plugin:", err.message);
  ...
  function syncHermesPlugin() {
    try {
      ...
      if (result && result.status === "error") {
        console.warn("Nibbo: failed to sync Hermes plugin:", result.message);
        return result;
      }
      if (result && (result.installed > 0 || result.updated > 0)) {
        console.log(`Nibbo: synced Hermes plugin (installed=${result.installed}, updated=${result.updated})`);
      }
      return result && typeof result === "object" ? result : { status: "ok" };
    } catch (err) {
      console.warn("Nibbo: failed to sync Hermes plugin:", err.message);
  ...
  function syncQoderHooks() {
    try {
      ...
      if (added > 0 || updated > 0) {
        console.log(`Nibbo: synced Qoder hooks (added ${added}, updated ${updated})`);
      }
      return { status: "ok", added, updated };
    } catch (err) {
      console.warn("Nibbo: failed to sync Qoder hooks:", err.message);
