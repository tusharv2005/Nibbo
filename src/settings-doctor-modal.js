...
(function initSettingsDoctorModal(root) {
  ...
  function renderFixButton(core, action) {
    ...
    const restart = action.type === "restart-nibbo";
    const label = busy
  ...
  function requiresFixConfirmation(action) {
    ...
    if (action.type === "restart-nibbo") return true;
    return !!(
  ...
  function mountModal(core, result) {
    ...
    if (openLog) {
      openLog.addEventListener("click", async () => {
        try {
          if (!root.doctor || typeof root.doctor.openNibboLog !== "function") throw new Error(t(core, "doctorOpenLogFailed"));
          const result = await root.doctor.openNibboLog();
  ...
  async function startFixAction(core, action) {
    ...
    try {
      ...
      if (commandAction.type !== "restart-nibbo") delete commandAction.confirmed;
      ...
      // restart-nibbo tears the main process down right after this IPC reply,
      ...
      if (action && action.type === "restart-nibbo") return;
