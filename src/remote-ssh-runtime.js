...
const NIBBO_SERVER_HEADER = "x-nibbo-server";
const CLAWD_SERVER_ID = "nibbo";
...
//   1   header match + non-200    (local Nibbo server unhealthy)
...
function buildProbeCommand(remoteForwardPort, nodeBin = "node") {
  ...
  const js =
    `const r=require('http').get(${JSON.stringify(url)},res=>{` +
      `const m=res.headers[${JSON.stringify(NIBBO_SERVER_HEADER)}]===${JSON.stringify(CLAWD_SERVER_ID)};` +
      `if(!m)process.exit(3);` +
