...
const LEGACY_APPROVAL_CALLBACK_RE = /^nibboperm:([a-z0-9]+):(allow|deny)$/;
...
function createTelegramNativeRunner({
  ...
  async function handleTestCallback(cb, { fromId, chatId }) {
    ...
    if (cb.data !== `nibbo-test:${pendingTest.nonce}` || !isAllowedUser || !isExpectedChat) {
      if (typeof cb.data !== "string" || !cb.data.startsWith("nibbo-test:")) return false;
  ...
  async function sendTestCard() {
    ...
    try {
      const msg = await client.sendMessage({
        chat_id: chatId,
        text: "Nibbo: test native Telegram bot. Tap to confirm.",
        reply_markup: {
          inline_keyboard: [[{ text: "Confirm", callback_data: `nibbo-test:${nonce}` }]],
        },
      });
