...
const IMPORT_MARKER_FILENAME = ".nibbo-imported-pet.json";
const ERR_REPLACE_DECLINED = "ERR_NIBBO_CODEX_PET_REPLACE_DECLINED";
...
function parseNibboImportUrl(rawUrl) {
  let parsed;
  try {
    parsed = new URL(String(rawUrl || ""));
  } catch {
    throw new Error("invalid nibbo import URL");
  }
  if (parsed.protocol !== "nibbo:") throw new Error("unsupported protocol");

  const action = parsed.hostname || parsed.pathname.replace(/^\/+/, "");
  if (action !== "import-pet") throw new Error(`unsupported nibbo action: ${action || "(missing)"}`);
...
async function downloadHttpsBuffer(rawUrl, options = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = request({
      ...
      headers: { "User-Agent": "Nibbo-Codex-Pet-Importer" },
  ...
  try {
    ...
    if (fs.existsSync(targetDir)) {
      ...
      const confirmed = await confirmReplaceExistingPackage({
        ...
        existingMarker,
        wasNibboImported: !!existingMarker,
      });
      if (!confirmed) {
...
function derivePackageDirName(manifest) {
  ...
  // separate ASCII-safe Nibbo theme ids from the installed package metadata.
