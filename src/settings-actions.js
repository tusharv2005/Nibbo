...
const updateRegistry = {
  ...
  showTray(value, { snapshot }) {
    ...
    if (!value && snapshot && snapshot.showDock === false) {
      return {
        status: "error",
        message: "Cannot hide Menu Bar while Dock is also hidden — Nibbo would become unquittable.",
      };
    }
    return { status: "ok" };
  },
  showDock(value, { snapshot }) {
    ...
    if (!value && snapshot && snapshot.showTray === false) {
      return {
        status: "error",
        message: "Cannot hide Dock while Menu Bar is also hidden — Nibbo would become unquittable.",
      };
    }
    return { status: "ok" };
  },
