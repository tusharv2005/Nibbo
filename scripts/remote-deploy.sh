...
if [ -n "$HOST_PREFIX" ]; then
  ...
  if printf '%s' "$HOST_PREFIX" | ssh "$SSH_TARGET" "cat > ~/.claude/hooks/nibbo-host-prefix"; then
    echo "  [OK] Prefix written to ~/.claude/hooks/nibbo-host-prefix"
  else
